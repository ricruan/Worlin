...
class WrProblemVO(BaseModel):
    ...
    problem_content: Optional[bytes] = Field(alias="problemContent", description="问题内容")
    ...
    create_by: Optional[str] = Field(alias="createBy", max_length=64, description="创建者")
    create_time: Optional[datetime] = Field(alias="createTime", description="创建时间")
    update_by: Optional[str] = Field(alias="updateBy", max_length=64, description="更新者")
    update_time: Optional[datetime] = Field(alias="updateTime", description="更新时间")
    del_flag: Optional[str | int] = Field(alias="delFlag", description='逻辑删除标识')
